export default function Navbar() {
  return (
    <nav className="bg-red-600/90 backdrop-blur-md p-4 flex justify-between items-center shadow-md shadow-red-900">
      {/* Logo & Title */}
        ...
        {["drivers", "teams", "circuits", "races", "standings"].map((item) => (
          <li key={item}>
            <a href={`/${item}`} className="relative group">
              <span className="hover:text-black transition capitalize">{item}</span>
